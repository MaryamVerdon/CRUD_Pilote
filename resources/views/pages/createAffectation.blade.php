...
<div class="h-screen bg-gradient-to-br bg-gray-100 flex justify-center items-center w-full">
    <form method="post" action="{{ route('add_affectation')}}">
        @csrf
      <div class="bg-white px-10 py-8 rounded-xl w-screen shadow-md max-w-sm">
        ...
        @endif
          ...
          <div>
            ...
            <input type="number" min="1" name="nbPassager" class="bg-indigo-50 px-4 py-2 outline-none rounded-md w-full" />
          </div>
          <div>
            <label for="vol" class="block mb-1 text-gray-600 font-semibold">Date vol</label>
            <input type="date" name="dateVol" class="bg-indigo-50 px-4 py-2 outline-none rounded-md w-full" min=/>
          </div>
